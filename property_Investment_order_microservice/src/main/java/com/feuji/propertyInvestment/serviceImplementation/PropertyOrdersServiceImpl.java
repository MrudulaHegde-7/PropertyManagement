...
//		Customer customer = restTemplate.getForObject("http://localhost:8081/api/customer/{cid}",Customer.class,cid);		   
//		//setted customer
//		orders.setCustomerId(customer);	
//		Property property = restTemplate.getForObject("http://localhost:8080/api/property/{pid}",
//				Property.class,pid);
//		
//		System.out.println(property);
//		System.out.println(property.getRemainingUnits());
...
//		restTemplate.postForObject("http://localhost:8080/api/property/{adminId}",property,Property.class,Map.of("adminId",adminId));
...
//			System.out.println(property.getRemainingUnits());
	...
	public PropertyOrder save(PropertyOrder orders, int cid, int pid) {
		...
		Customer customer = restTemplate.getForObject(
				"http://" + EntityUtil.getUrl() + ":9041/api/customer/customer/{cid}", Customer.class, cid);
		// setted customer
		orders.setCustomerId(customer);
		...
		// int
		// adminId=property.getAdmins().get(property.getAdmins().size()-1).getAdminId();
		// setting property
		int adminId = 1;
		orders.setPropertyId(property);
		restTemplate.postForObject("http://" + EntityUtil.getUrl() + ":9044/api/property/{adminId}", property,
				Property.class, Map.of("adminId", adminId));
		propertyOrdersRepositary.saveAndFlush(orders);
		return orders;
	...
	public void update(PropertyOrder orders, int cid, int pid) {
		// restTemplate.getMessageConverters().add(new
		// MappingJackson2HttpMessageConverter());
		Customer customer = restTemplate.getForObject("http://" + EntityUtil.getUrl() + ":9041/api/customer/customer/{cid}",
				Customer.class, cid);
		orders.setCustomerId(customer);
		Property property = restTemplate.getForObject("http://" + EntityUtil.getUrl() + ":9044/api/property/{pid}",
				Property.class, pid);
